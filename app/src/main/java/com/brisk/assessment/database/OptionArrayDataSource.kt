...
class OptionArrayDataSource private constructor() {

    companion object {
        private var hasObject = false
        private var myDataSource: OptionArrayDataSource? = null
        private val TAG = OptionArrayDataSource::class.java.simpleName
        val instance: OptionArrayDataSource?
            get() = if (hasObject) {
                myDataSource
            } else {
                hasObject = true
                myDataSource = OptionArrayDataSource()
                myDataSource
            }
    ...
    private fun cursorData(cursor: Cursor): OptionRes {
        val optionRes = OptionRes()
    ...
    fun deleteAll(context: Context?) {
        synchronized(lock) {
            try {
                val db = getSQLiteDb(true, context)
                db.execSQL("DELETE FROM " + DatabaseHelper.option_mst)
    ...
    fun getOptionArrayData(sqId : String,context: Context?): ArrayList<OptionRes> {
        synchronized(lock) {
            val optionRes : ArrayList<OptionRes> = ArrayList<OptionRes>()
            ...
            try {
                val cursor = db.rawQuery(
                    "SELECT * FROM " + DatabaseHelper.option_mst + " WHERE " + DatabaseHelper.subQuestionId + " = '" + sqId + "'",
                    null
                )
                if (cursor.moveToFirst()) {
                    do {
                        optionRes.add(cursorData(cursor))
